...
import { getCurrentDay, addWeeks } from '../utils/dateUtils';
...
const NextWeekFocus: React.FC<NextWeekFocusProps> = ({
  ...
  const nextWeekStart = startOfWeek(addWeeks(today, 1), { weekStartsOn: 1 }); // Start on Monday
  const nextWeekEnd = endOfWeek(addWeeks(today, 1), { weekStartsOn: 1 }); // End on Sunday

  useEffect(() => {
    loadNextWeekProjects();
  }, [selectedDepartment]);

  const loadNextWeekProjects = async () => {
    try {
      setLoading(true);
      ...
      const processedProjects = (flowChartProjects || [])
        ...
        .filter((project: ProcessedProject) => {
          // Filter projects that are active during next week
          const hasEndDate = project.startDate.getTime() !== project.endDate.getTime();
          
          if (!hasEndDate) {
            // For ongoing projects (no end date), include if start date is before or during next week
            return project.startDate <= nextWeekEnd;
          } else {
            // For projects with end dates, use the existing logic
            return isWithinInterval(nextWeekStart, { start: project.startDate, end: project.endDate }) ||
                   isWithinInterval(nextWeekEnd, { start: project.startDate, end: project.endDate }) ||
                   (project.startDate <= nextWeekStart && project.endDate >= nextWeekEnd);
          }
        });
      ...
      console.error('Error loading next week projects:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        >
          <div className="flex items-center">
            <CalendarDays size={18} className="mr-2" style={{ color: 'white' }} />
            <h3 
        ...
        <div className="flex items-center justify-center py-8 p-4">
          ...
          <p style={{ color: brandTheme.text.secondary }}>Loading next week's focus...</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      >
        <div className="flex items-center">
          ...
          <CalendarDays size={18} className="mx-2" style={{ color: 'white' }} />
          <h3 
            className="text-lg font-semibold"
            style={{ color: 'white' }}
          >
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            {filteredProjects.length} upcoming project{filteredProjects.length !== 1 ? 's' : ''}
          </span>
          <FilterButton
            users={users}
      ...
      {isExpanded && (
        <div className="p-4">
          {filteredProjects.length === 0 ? (
            ...
            >
              <CalendarDays size={32} className="mx-auto mb-2 opacity-50" style={{ color: brandTheme.text.muted }} />
              <p className="text-sm" style={{ color: brandTheme.text.muted }}>
                No active projects next week.
              </p>
            </div>
          ) : (
