...
interface Sprint2ColumnProps {
  onProjectClick?: (project: any) => void;
  onSprintReviewClick?: (project: any) => void;
  refreshTrigger?: number; // Used to trigger refresh from parent
}

const Sprint2Column: React.FC<Sprint2ColumnProps> = ({ onProjectClick, onSprintReviewClick, refreshTrigger }) => {
  ...
  const fetchSprintGroupCount = async () => {
    try {
      const { data, error } = await (supabase as any)
        .from('PMA_Sprints')
        .select('id')
        .eq('sprint_type', 'Sprint 2')
        .eq('status', 'active');

      if (error) {
        console.error('Error fetching Sprint 2 group count:', error);
        return;
      }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="rounded-t-lg border-t-8" style={{ borderTopColor: brandTheme.status.info }}>
        ...
        >
          <h3 className="text-lg font-bold text-white">Sprint 2</h3>
        </div>
      </div>
