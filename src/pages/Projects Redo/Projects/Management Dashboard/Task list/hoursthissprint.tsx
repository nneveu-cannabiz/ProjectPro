...
interface HoursThisSprintProps {
  isOpen: boolean;
  onClose: () => void;
...
const HoursThisSprint: React.FC<HoursThisSprintProps> = ({ 
  isOpen, 
  onClose, 
  sprintStartDate, 
  sprintEndDate, 
  ...
  const weeklyData = useMemo((): WeekData[] => {
    // Filter hours for sprint date range only
    const sprintHours = allHoursData.filter(entry => {
      const entryDate = parseISODate(entry.date);
      return entryDate >= sprintStartDate && entryDate <= sprintEndDate;
    });

    // Group by weeks
    const weekMap = new Map<string, WeekData>();

    sprintHours.forEach((entry) => {
      const entryDate = parseISODate(entry.date);
    ...
    return weeks;
  }, [allHoursData, sprintStartDate, sprintEndDate]);
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Hours This Sprint</h2>
              <p className="text-sm text-white opacity-90">{sprintLabel}</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p style={{ color: brandTheme.text.muted }}>No hours logged this sprint</p>
              </div>
            </div>
          ) : (
                            ...
                            <tbody>
                              <tr>
                                {week.days.map((day, dayIndex) => {
                                  // Check if this day is outside the sprint date range
                                  const isOutsideSprint = day.date < sprintStartDate || day.date > sprintEndDate;
                                  ...
                                  >
                                    {day.users.length === 0 ? (
                                      <div className="text-center text-xs py-4" style={{ color: brandTheme.text.muted }}>
                                        {isOutsideSprint ? 'Outside Sprint' : 'No hours'}
                                      </div>
                                    ) : (
...
export default HoursThisSprint;
